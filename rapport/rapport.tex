...
Afin d'améliorer la sécurité de la CPRF $F^{WEAK}$, il est possible de faire une implémentation en utilisant une fonction de hachage $H$. Cet implémentation apporte une modification sur l'évaluation d'une entrée contrainte ou non, sans empêcher un attaquant de demander une clé contrainte.
...
\paragraph{\'{E}valuation sur les entrées contraintes : }Après avoir reçue une clé contrainte $S_y$, pour évaluer un vecteur $\vec{x}$ orthogonal à $\vec{y}$, l'attaquant calculera $EvalC_H(\vec{x}) = H(S_y \cdot \vec{x})$ au lieu de $EvalC(\vec{x}) = S_y \cdot \vec{x}$, où $H$ est une fonction de hachage.

Cette implémentation qui utilise une fonction de hachage $H$, assure qu'un attaquant puisse toujours faire des évaluations sur les entrées contraintes, mais il ne pourras pas reconstituer la matrice $S$ en résolvant des systèmes linéaires. En effet, en utilisant l'attaque décrite dans la section précedente, retrouver $S$ revient à chercher une pré-image $H(S \cdot \vec{x})$. Ceci confirme que l'application d'une fonction de hachage assure la sécurité de la CPRF $F^{WEAK}$
...
\section{Annexe : Graphiques et guide d'utilisation}
...
\hline
\texttt{make} & Compilation \\
\hline
\texttt{./start.sh -n <size>} & CPRF normale \\
\hline
\texttt{./start.sh -h <size>} & CPRF hachée (SHA-256) \\
\hline
\texttt{./start.sh -l <size>} & Lazy sampling \\
\hline
\texttt{./start.sh -f <tries> <N> <M>} & Mode fweak \\
\hline
\texttt{python3 display.py} & Affichage des courbes \\
\hline
\end{tabular}
\end{center}
