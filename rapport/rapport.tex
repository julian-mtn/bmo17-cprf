...
\tcbset{ %pour les boîtes de définitions
  mydefstyle/.style={
    colback=white,
    colframe=black,
    fonttitle=\bfseries,
...
\tikzset{ %pour les chaine de permutations
  state/.style={
    draw,
    rectangle,
    rounded corners,
...
\begin{tikzpicture}[node distance=3cm]
  ...
  \node[draw, rectangle, rounded corners, minimum height=8mm, minimum width=14mm, align=center, right of=dots] (stx) {$ST_a$};
  ...
  \draw[->, thick, dashed, red] (stx.south) .. controls +(0,-2.2) and +(0,-2.2) .. 
    node[below, red, align=center] {Attaquant applique la permutation inverse \\ pour vérifier $ST_a = ST_n$} (stn.south);
...
L’idée est d’introduire une fonction de hachage cryptographique dans l'évaluation de la CPRF. 
...
\subsection{Implémentation de la CPRF hachée}

Pour implémenter cette version hachée, nous avons utilisé la fonction de hachage SHA-256 d’OpenSSL.
L’évaluation de la CPRF est effectuée normalement, puis le résultat est converti en bytes et est haché avant d’être retourné à l’attaquant.
De même pour l’évaluation avec la clé contrainte.
...
\begin{figure}[H]
  ...
  \caption{Attaque version hachée}
\end{figure}
