...
\subsection{CPRF avec une permutation linéaire $F^{WEAK}$}
...
\end{cases}
\]
Cette construction permet donc de restreindre l’évaluation de la fonction aux seuls vecteurs orthogonaux à $\vec{y}$. Lorsque l'attaquant envoie un vecteur $\vec{y}$, il es impératif que l'oracle doit refuser si $\vec{y} = \vec{0}$, sinon l'oracle va envoyer $S_{\vec{y}} = S$, ce qui révèlera la clé maîtresse à l'attaquant.
...
Pour un $\vec{x}$ orthogonal à $\vec{y}$, pour chaque $\vec{Si}$, notons $k_i = \vec{Si} \cdot \vec{x} = \vec{Si}_{\vec{y}} \cdot \vec{x}$.
...
%TODO expliquer qu'on peut quand même avoir de l'information et distinguer une sortie aléatoire de la CPRF
...
Cet alternative permet donc de reconstruire chaque élément $S_{i,j}$ ligne  de la matrice $S$ en fonction d'un seul élément $S_{i,N}$. Ceci permet à l'attaquant, à chaque requête envoyer à l'oracle, de retrouver une valeur de $S_{i,N}$, et de garder les valeurs de $S_{i,N}$ qui se répètent le plus comme la vraie valeur de $S_{i,N}$.
...
%TODO : Trouver une attaque qui permet de distingue une fonction aléatoire de l'application de $S$ à un vecteur à partir de cette reconstruction de $S$.
...
Pour reconstruire la matrice $X \in (\mathbb{Z}/p\mathbb{Z})^{(N-1) \times (N-1)}$, il peut alors choisir choisir un vecteur $\vec{x} = (0,\ldots,0,1,0,\ldots,0) \in (\mathbb{Z}/p\mathbb{Z})^{N-1}$, pour la ligne $j$, où on a un $1$ au $j^{\text{ième}}$ élément du vecteur.

Ainsi, dans le système linéaire précédent, on a $X = Id_{N-1}$ et $\forall j, x_{j,N} = 0$. On se retrouve donc avec le système linéaire $X \cdot \vec{S_i}^T = \vec{k_i} \Leftrightarrow Id_{N-1} \cdot \vec{S_i}^T = \vec{k_i}$. Ceci permet de retrouver les $N-1$ premières colonnes de $S$ immédiatement. En effet, dans ce cas particulier, $S_{i,j} = k_{i_j}$.

\subsection{Jeu de sécurité pour l'attaque sur $(F^{WEAK})$}

Formellement, voici une attaque qu'un attaquant utilisera pour distinguer une CPRF $F^{WEAK}$ d'une CPRF :
...
%\item $\mathcal{A}$ construit la matrice $X \in (\mathbb{Z}/p\mathbb{Z})^{(N-1) \times (N-1)}$ où la $j^{\text{ième}}$ ligne est $\{(0,\ldots,0,1,0,\ldots,0)\}$, donc $X = Id_{N-1}$.
...
Dans ce jeu de sécurité, à chaque demande d'évaluation d'un vecteur $\vec{x}$, l'attaquant recalcule les valeurs de $S_{1,N},\ldots,S_{N,N}$. Les valeurs redondantes ont une haute probabilité que ce sont les vraies valeurs de $S_{1,N},\ldots,S_{N,N}$, ce qui permet d'en déduire que le résultat de l'évaluation vient d'une CPRF. Si après une évaluation, on retrouve des valeurs de $S_{1,N},\ldots,S_{N,N}$ qui ne sont pas redontantes, alors c'est très probablement le résultat d'une fonction aléatoire.
...
\begin{defbox}{Jeu de sécurité pour \(F^{WEAK}\)}
L'attaquant \(\mathcal{A}\) interagit avec un oracle \(\mathcal{O}\) qui est soit la construction \(F^{WEAK}\)
définie par une matrice secrète \(S \in (\mathbb{Z}/p\mathbb{Z})^{M \times N}\), soit une fonction
aléatoire \(\mathcal{R} : (\mathbb{Z}/p\mathbb{Z})^{N} \rightarrow (\mathbb{Z}/p\mathbb{Z})^{M}\).
L’objectif de l'attaquant est de distinguer les deux cas.

\begin{itemize}
\item \textbf{1 - Demande de clé contrainte :}
\(\mathcal{A}\) choisit un vecteur \(\vec{y} \in (\mathbb{Z}/p\mathbb{Z})^{N}\) et interroge l’oracle
avec la requête \(\textsf{Constrain}(\vec{y})\). L’oracle génère alors une clé contrainte
\[
S_{\vec{y}} = S + \vec{d} \cdot \vec{y}^{T},
\]
où \(\vec{d} \in (\mathbb{Z}/p\mathbb{Z})^{M}\) est choisi uniformément et indépendamment
à chaque requête. \(\mathcal{A}\) reçoit \(S_{\vec{y}}\) et peut désormais évaluer la fonction
sur tout vecteur \(\vec{x}\) tel que \(\langle \vec{x}, \vec{y} \rangle = 0\).

\item \textbf{2 - Requêtes d’évaluation :}
\(\mathcal{A}\) interroge l’oracle \(\mathcal{O}\)
sur des vecteurs \(\vec{x} \in (\mathbb{Z}/p\mathbb{Z})^{N}\) de son choix.
\begin{itemize}
    \item Si \(\mathcal{O}\) est la construction \(F^{WEAK}\), l’oracle répond en utilisant la clé maîtresse \(S\) :
    \[
    \textsf{Eval}(S, \vec{x}) = S \cdot \vec{x}.
    \]
    \item Si \(\mathcal{O}\) est une fonction aléatoire \(\mathcal{R}\), l’oracle répond en tirant uniformément un vecteur aléatoire \(\vec{r} \in (\mathbb{Z}/p\mathbb{Z})^{M}\) et retourne \(\vec{r}\).
\end{itemize}

\item \textbf{3 - Exploitation des informations :}
\(\mathcal{A}\) utilise les informations collectées (clés contraintes et évaluations) pour
tenter de reconstruire partiellement la matrice \(S\) afin de distinguer les deux mondes.

Il sélectionne \(N-1\) vecteurs \(\{\vec{x}_j\}_{1 \leq j \leq N-1}\)
orthogonaux à \(\vec{y}\) et formant une famille libre. Pour chaque ligne \(S_i\) de \(S\),
il construit le système linéaire
\[
X \cdot \vec{S_i}'^{T} = \vec{k}_i,
\]
où \(X\) est la matrice des \(\vec{x}_j\), \(\vec{S_i}'\) est le vecteur \(S_i\) avec une valeur en moins, et \(\vec{k}_i\) est le vecteur des évaluations obtenues.

\item \textbf{4 -Décision :}
\(\mathcal{A}\) décide que \(\mathcal{O}\) est \(F^{WEAK}\) s’il arrive à reconstruire
une matrice cohérente avec les observations et/ou s’il détecte une structure linéaire.
Sinon, il décide que \(\mathcal{O}\) est une fonction aléatoire.
\end{itemize}
...
\subsection{Renforecement de \texorpdfstring{$F^{\text{WEAK}}$}{F WEAK} par hachage}

TODO

\subsection{Étude d'autres permutations alternatives}
