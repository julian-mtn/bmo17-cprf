...
\node[above=0.5cm of X] {Matrice $X$ construite avec $\{\vec{x_j}\}$ orthogonaux à $\vec{y}$};

\end{tikzpicture}
\caption{Matrice $X$ pour l'attaque}
\end{figure}
...
Ici, l'ensemble des vecteurs $\{\vec{x_j}\}_{1 \leq j \leq N-1}$ forme une famille libre, donc $rang(X) = N-1$, donc $X$ est inversible, donc ce système linéaire possède une unique solution. Cependant, cet unique solution est calculée en fonction de $S_{i,N}$.
...
\subsection{Étude de permutations alternatives}
