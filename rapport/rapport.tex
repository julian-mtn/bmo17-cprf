...
c’est-à-dire une valeur $ST_x = F((SK, ST_0), x). $. À cet étape, l'attaquant ne sait pas si $ST_x$ est le résultat d'une fonction aléatoire ou non.
...
\begin{itemize}
\item \textbf{Phase 1 :} $\mathcal{A}$ demande une clé contrainte $K_C$ pour une contrainte de son choix, lui permettant d’évaluer la fonction sur un sous-ensemble d’entrées.
\item \textbf{Phase 2 :} $\mathcal{A}$ fait un nombre de requêtes d’évaluation à l’oracle $O$ sur des entrées de son choix
\item \textbf{Décision :} $\mathcal{A}$ décide si $O$ est la $CPRF$ ou la fonction aléatoire $G$.
\end{itemize}
\end{defbox}
