...
Plus précisément, la fonction publique correspond à l’exponentiation modulaire
\[
x \mapsto x^e \bmod N,
\]
tandis que l’inverse de la permutation est calculé via l’exponentiation privée
\[
x \mapsto x^d \bmod N.
\]
...
d’exécution indépendant des données secrètes.
...
L’attaque CJ25 exploite le fait qu'un attaquant peut reçevoir des évaluations et évaluer en parallèle des entrées à partir de clés contraintes.

Plus précisément, l’adversaire commence par obtenir une évaluation de la fonction pseudo-aléatoire contrainte sur une entrée donnée $x$, 
c’est-à-dire une valeur $y = F(K, x). $

Dans un second temps, il peut demander une ou plusieurs clés contraintes $K_R$ telles que l’ensemble de contraintes $R$ inclut l’entrée $x$.
Àvec une telle clé contrainte, il peut alors recalculer la valeur 
$y' = \mathrm{EvalC}(K_R, x)$, et tester si $y' = y$. 

Il est donc possible pour un attaquant de discerner les sortie réelles des sorties aléatoires de la CPRF.
