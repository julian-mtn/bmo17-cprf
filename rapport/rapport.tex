...
\node[above=0.5cm of X] {Matrice $X$ construite avec $\{\vec{x_j}\}$ orthogonaux à $\vec{y}$, $rang(X)=N$};
...
On répètera $M$ fois ce procédé pour retrouver chacun des $\vec{Si}$.

Or, ce système linéaire possède une unique solution si et seulement si la matrice $X$ est inversible, ce qui est équivalent à $rang(X) = N$.
Pour trouver une solution à chacun de ces système, il faut donc choisir des vecteurs $\vec{x_j}$ tel que $\{\vec{x_j}\}_{1 \leq j \leq N}$ est une famille libre, ce qui est équivalent au fait que $\{\vec{x_j}\}_{1 \leq j \leq N}$ génère un espace de dimension $N$.
Et comme il est nécessaire que tous les vecteurs $\vec{x_j}$ sont orthogonaux à $\vec{y}$, l'ensemble $\{\vec{y}\} \cup \{\vec{x_j}\}_{1 \leq j \leq N}$ génère un espace de dimension $N+1$.

Cela est absurde car les vecteurs $\vec{x_j}$ et $\vec{y}$ sont composés de $N$ éléments
