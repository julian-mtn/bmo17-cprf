...
\subsection{Rabin trapdoor permutation}

\paragraph{}The Rabin permutation is a trapdoor function based on the hardness of integer factorization. It relies on the squaring operation modulo a composite integer.
...
\item\textbf{Inverting the Rabin permutation:}
...
Even with the secret key (\(p,q\)), it is possible to compute all the roots but \textbf{not directly determine which one corresponds to the original message}.
Applying the inverse \(n-c\) times does not guarantee recovering the original message.
...
\node[correct] {$ST_0$}
  ...
  child {node {$X_3$}};
\end{tikzpicture}
\caption{Tree of preimages in the Rabin permutation.}\label{fig:rabin-tree}
\end{figure}

\paragraph{Conclusion:}
The Rabin permutation is not suitable for our CPRF because each value has four possible preimages. 
Even with the private key, it is impossible to know which one corresponds to the original input, which prevents 
the repeated application of the inverse $(n-c)$ times from correctly recovering the CPRF output.
