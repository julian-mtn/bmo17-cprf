...
\small Master 1 Cryptologie, Calcul Haute-Performance et Algorithmes\\
...
Les fonctions pseudo-aléatoires (\emph{Pseudorandom Functions}, PRF) sont des primitives
fondamentales en cryptographie moderne. Elles interviennent dans de nombreux protocoles,
notamment pour le chiffrement symétrique, l’authentification ou la dérivation de clés.
Dans certains contextes, il est nécessaire de déléguer uniquement une partie des capacités
d’évaluation d’une PRF sans révéler la clé secrète complète. Ce besoin apparaît notamment dans le contexte de la recherche sur données chiffrées
(\emph{Searchable Symmetric Encryption}) et du stockage externalisé sur des serveurs non fiables,
où il est nécessaire de contrôler précisément les capacités d’évaluation d’un adversaire~\cite{BMO17}.
Les fonctions pseudo-aléatoires contraintes (\emph{Constrained Pseudorandom Functions}, CPRF)
répondent à ce problème en permettant de restreindre l’évaluation de la fonction à un sous-ensemble
d’entrées défini par une contrainte~\cite{BMO17}.

Ce projet s’intéresse à la construction de CPRF proposée par
Bost, Minaud et Ohrimenko en 2017~\cite{BMO17}, basée sur l’itération
d’une permutation à trappe.
Dans un premier temps, nous présentons les notions fondamentales liées aux fonctions
pseudo-aléatoires et aux CPRF, ainsi que la construction proposée dans~\cite{BMO17}.
Dans un second temps, nous étudions plusieurs variantes de cette construction reposant sur RSA, Rabin, AES,
ainsi qu’une alternative basée sur des transformations linéaires,
$F^{\text{WEAK}}$, dont nous proposons une version hachée et pour laquelle
nous discutons la sécurité.
Enfin, nous revenons sur les travaux récents de Cheng et Jaeger~\cite{CJ25},
qui mettent en évidence une attaque contre cette CPRF et proposent une variante hachée
permettant de renforcer sa sécurité.

L’ensemble du projet est accompagné d’une implémentation complète et de simulations
permettant d’évaluer expérimentalement les attaques étudiées.

\small \textit{Le code source associé au projet est disponible sur GitHub : 
...
\subsection{CPRF avec la permutation à trappe RSA}
...
\section{Étude d'autres permutations alternatives}

La construction de BMO17 repose sur l’utilisation d’une permutation à trappe
itérable avec RSA. Il est toutefois intéressant
d’étudier d’autres constructions possibles afin de mieux comprendre les propriétés
nécessaires à la sécurité d’une CPRF.

Nous étudions donc plusieurs alternatives à la permutation RSA :
une construction linéaire $F^{\text{WEAK}}$, une basée sur Rabin
ainsi qu’une approche utilisant AES.

\subsection{Permutation linéaire $F^{WEAK}$}
...
\subsubsection{Implémentation de la permutation}
Ici, l'évaluation de la permutation à trappe est définie par :
\[Eval(F^{WEAK},\vec{x}) = Eval(S,\vec{x}) = S \cdot \vec{x}\] 

où $S$ est une matrice de taille $M \times N$ à coefficients dans $\mathbb{Z}/p\mathbb{Z}$ où $M << N$.

$S$ est la clé maîtresse et elle est choisie aléatoirement lors de la génération de la clé maîtresse.

\subsubsection{Implémentation de la clé contraintes}

Pour générer la clé contrainte :
\begin{itemize}
  \item l'attaquant envoie un vecteur $\vec{y} \in (\mathbb{Z}/p\mathbb{Z})^N$
  \item l'oracle tire au hasard un vecteur $\vec{d} \in (\mathbb{Z}/p\mathbb{Z})^M$ tel que $\vec(d) \neq \vec(0)$
  \item l'oracle renvoie $Constrain(S, \vec{y}) = S_{\vec{y}} = S + d \cdot \vec{y}^T$
\end{itemize}
...
on remarque que l'ensemble des vecteurs contraints sont les $\vec{x} \in (\mathbb{Z}/p\mathbb{Z})^N $ tel que $<\vec{x},\vec{y}> = 0$.

En effet,

\[CEval(S_{\vec{y}}, \vec{x}) =  S_{\vec{y}} \cdot \vec{x}\]
\[CEval(S_{\vec{y}}, \vec{x}) =  S \cdot \vec{x} + d \cdot \vec{y}^T \cdot \vec{x}\]
\[CEval(S_{\vec{y}}, \vec{x}) =  S \cdot \vec{x} + d \cdot <\vec{x},\vec{y}>\]
\[CEval(S_{\vec{y}}, \vec{x}) =  S \cdot \vec{x} \iff <\vec{x},\vec{y}> = 0\]

Donc :
\[
S_{\vec{y}} \cdot \vec{x} =
\begin{cases}
S \cdot \vec{x} & \text{si } \langle \vec{x}, \vec{y} \rangle = 0 \\
\text{résultat 'brouillé'} & \text{sinon}
\end{cases}
\]
Cette construction permet donc de restreindre l’évaluation de la fonction aux seuls vecteurs orthogonaux à $\vec{y}$. Lorsque l'attaquant envoie un vecteur $\vec{y}$, il est impératif que l'oracle doit refuser si $\vec{y} = \vec{0}$, sinon l'oracle va envoyer $S_{\vec{y}} = S$, ce qui révèlera la clé maîtresse à l'attaquant.


\subsubsection{Approche d'une attaque à base d'un système linéaire}

Notons $\vec{Si} \in (\mathbb{Z}/p\mathbb{Z})^N$ la ième ligne de $S$.

Pour un $\vec{x}$ orthogonal à $\vec{y}$, pour chaque $\vec{Si}$, notons $k_i = \vec{Si} \cdot \vec{x} = \vec{S}_{\vec{y}_i} \cdot \vec{x}$.
...
\end{tikzpicture}
\caption{Représentation du système linéaire $S \cdot \vec{x} = \vec{k}$}
\end{figure}


En choisissant $N$ vecteurs $\{\vec{x_j}\}_{1 \leq j \leq N}$ tel que $<\vec{x_j},\vec{y}> = 0, \forall j$, on peut construire la matrice $X \in (\mathbb{Z}/p\mathbb{Z})^{N \times N}$ tel que la jème ligne de $X$ soit $\vec{x_j}^T$
...
\end{tikzpicture}
\caption{Matrice $X$ pour l'attaque construite avec $\{\vec{x_j}\}$ orthogonaux à $\vec{y}$}
\end{figure}
...
\textbf{\'{E}valuation sur les entrées non contraintes : }Lorsqu'un attaquant demande l'évaluation d'un vecteur $\vec{x} \in (\mathbb{Z}/p\mathbb{Z})^N$, alors l'oracle lui enverrait $Eval_H(\vec{x}) = H(S \cdot \vec{x})$ au lieu de $Eval(\vec{x}) = S \cdot \vec{x}$, où $H$ est une fonction de hachage.

\textbf{Demande d'une clé contrainte : }La demande de la clé contrainte par un attaquant reste inchangée. L'attaquant envoie un vecteur $\vec{y} \in (\mathbb{Z}/p\mathbb{Z})^N$, puis l'oracle renvoie $Constrain(S, \vec{y}) = S_{\vec{y}} = S + d \cdot \vec{y}^T$.

\textbf{\'{E}valuation sur les entrées contraintes : }Après avoir reçu une clé contrainte $S_y$, pour évaluer un vecteur $\vec{x}$ orthogonal à $\vec{y}$, l'attaquant calculera $EvalC_H(\vec{x}) = H(S_y \cdot \vec{x})$ au lieu de $EvalC(\vec{x}) = S_y \cdot \vec{x}$, où $H$ est une fonction de hachage.
...
\begin{itemize}
\item \textbf{Vérification si $\min_x |\textsf{F}^{\text{WEAK}}\textsf{.Out}(\lambda, x)|$ est super-polynomial en $\lambda$ :}
...
\item \textbf{Vérification si l'oracle aléatoire utilisé est non-programmable :}
...
\item \textbf{Vérification si $F^{\text{WEAK}}$ est IND$^*$-NC-CPRF sécurisée :}
...
\subsection{Fonction à trappe de Rabin}

La fonction de Rabin est une fonction à trappe fondée sur la difficulté
de la factorisation des entiers. Elle repose sur l’opération de mise au carré
modulo un entier composé.

Contrairement à RSA, cette fonction n’est pas injective et ne définit donc
pas une permutation à trappe.
...
\item\textbf{Fonction de Rabin :}

La fonction de Rabin :
\[
...
\item\textbf{Inverser la fonction de Rabin :}

Inverser la fonction de Rabin revient à résoudre l'équation :
\[
x^2 \equiv y \pmod{n}
\]
...
Appliquer l'inverse \(n-c\) fois n'assure pas de retrouver la pré-image originale.
...
\node[correct] {$ST_0$}
  ...
  child {node {$X_3$}};
\end{tikzpicture}
\caption{Arbre des pré-images dans la fonction de Rabin.}\label{fig:rabin-tree}
\end{figure}

\paragraph{Conclusion :}
La fonction de Rabin n’est pas adaptée à notre construction de CPRF,
car elle ne définit pas une permutation bijective. Chaque valeur a quatre pré-images possibles. 
...
Notons que nous avons tenté d'appliquer la transformation de Cheng et Jaeger à notre construction $F^{\text{WEAK}}$, afin d'obtenir une CPRF avec des contraintes plus expressives.
Malheureusement, cette approche échoue, le théorème de CJ25 requiert que la CPRF sous-jacente soit IND$^*$-NC-CPRF sécurisée, 
ce que $F^{\text{WEAK}}$ ne satisfait pas.
...
Après avoir implémenté différentes variantes de la CPRF (version originale, version hachée avec SHA-256 et version hachée avec lazy sampling), 
...
Toutes les expériences ont été réalisées sur la configuration matérielle suivante.
...
Processeur & Intel Core i7-1165G7 (4 cœurs, 8 threads) \\
Fréquence CPU & 2.80\,GHz (jusqu’à 4.70\,GHz) \\
Mémoire vive & 7.5\,GB RAM \\
Système d’exploitation & Ubuntu 24.04.4 LTS \\
Noyau Linux & 6.8.0 \\
Architecture & x86\_64 \\
Accélération matérielle & Aucune (pas de GPU utilisé) \\
\hline
\end{tabular}

\caption{Configuration matérielle utilisée pour les expériences.}
\label{fig:setup}
\end{figure}


La figure \ref{fig:stat1} présente les résultats obtenus sur la version non hachée de la CPRF. 
...
Dans les figures \ref{fig:stat2} et \ref{fig:stat3}, la même attaque est appliquée aux versions hachées (SHA-256 et lazy sampling). On observe alors un échec systématique de l’attaque, avec un taux de succès proche de 0.5, correspondant à un comportement équivalent à une décision aléatoire. Dans ces cas, l’attaquant ne parvient plus à exploiter la structure de la CPRF.

Sur la figure \ref{fig:stat4}, on observe les résultats de l'attaque qu'on a implémenter sur la cprf $F^{\mathrm{WEAK}}$, où on peut bien voir que cet attaque permet de gagner un avantage significatif sur la CPRF $F^{\mathrm{WEAK}}$ lorsqu'elle n'est pas haché.
